...
$qry_ex = "SELECT app.application_status as pre_login_status, city.city_name as city_name, cust.phone_no as phone, cust.name as name, app.id as app_id, cust.id as cust_id, user.name as user_name, qry.loan_type_id as loan_type,qry.loan_amount as required_loan_amt, app.login_date as login_date_on, app.sanction_date as sanction_date_on, app.disburse_date as first_disb_date_on, app.follow_up_date AS fup_date_on from crm_query_application as app JOIN crm_query as qry ON app.crm_query_id = qry.id Inner JOIN crm_customer as cust ON qry.crm_customer_id = cust.id left join crm_master_user as user on qry.lead_assign_to = user.id left join crm_master_city as city on cust.city_id = city.id ";
