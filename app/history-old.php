...
if(!in_array($user_role,array(1,2,3,4,5))){
...
if($user_role == 3){
    $qry .= " and hist.user_id = '".$user."' ";
}
if($user_role == 2 || $user_role == 4){ 
  if($user_role == 2){
  	$qry .= " and hist.user_id IN (".$tl_member.")";
  }
...
if($user_fetch != '' ) {
    $qry .= " and hist.user_id = '".$user_fetch."'";   
}
    ...
    if($result_app_history_query['case_status'] > 0){
      ...
      $check_mint_case_follow = mysqli_query($Conn1,"select app_id from tbl_mint_case_followup  where case_id=".$result_app_history_query['case_id']." order by follow_id DESC LIMIT 1");
              ...
              if($total_application > 0){
                $count=0;
                  while($result_application_qry = mysqli_fetch_assoc($get_no_of_app)){
                    ...
                    if($count == 0){
                      $count_imp_0 .= "<td style='background-color:".$bc_color.";".$borde."border-bottom: 1px solid black;'>".$result_application_qry['app_id']."</td><td style='background-color:".$bc_color.";".$borde."border-bottom: 1px solid black;'>".$result_application_qry['bank_name']."/ ".$result_application_qry['partner_name']."<br>".$app_follow_date."</td><td style='border-bottom: 1px solid black;background-color:".$bc_color.";".$borde."'>".$result_application_qry['app_description']."</td><td style='border-bottom: 1px solid black;background-color:".$bc_color.";".$borde."'>".$app_created_by."<br>".date('d-m-Y',strtotime($result_application_qry['la_st_up_date']))."</td><td style='border-bottom: 1px solid black;background-color:".$bc_color.";".$borde."'></td><td style='border-bottom: 1px solid black;background-color:".$bc_color.";".$borde."'>".$final_status."</td><td style='border-bottom: 1px solid black;background-color:".$bc_color.";".$borde."'><a href='edit_applicaton.php?case_id=".base64_encode($result_app_history_query['case_id'])."&amp;app_id=".base64_encode($result_application_qry['app_id'])."&amp;cust_id=".base64_encode($result_app_history_query['cust_id'])."&amp;loan_type=".$result_app_history_query['loan_type']."&all_app=1' target='_blank' class='has_link'><input type='button' class='pointer_n' value='View' style='border-radius: 5px; background-color: #18375f; font-weight: bold;'></a></td>";
                    }else{
          ...
          ?>
            ...
            <tr>
              <td rowspan="<?php echo $total_application; ?>"><?php echo $result_app_history_query['case_id']."<br>".$result_app_history_query['name']."<br>".$result_app_history_query['city_name']; ?></td>
              <td rowspan="<?php echo $total_application; ?>"><?php echo $case_status."<br>".$case_fup_date." ".$c_follow_time; ?></td>
