...
$qry_ex = "SELECT fb.id as fb_id,app.bank_crm_lead_on as bank_crm_lead_on,city.city_name as city_name,cust.phone as phone,cust.name as name,c.auto_case_create as auto_case_create,app.case_id as case_id,app.app_id as app_id,bank.bank_name as bank_name,app.app_status_on as app_status_on,c.cust_id as cust_id,user.user_name as user_name,loan.loan_type_name as loan_type_name,c.loan_type as loan_type,c.required_loan_amt as required_loan_amt,partner.partner_name as partner_name,app.bank_app_no_on as bank_app_no_on,app.disb_email_flag as disb_email_flag,pre_l.pre_status_name as pre_status_name,app.app_bank_on as app_bank_on,app.login_date_on as login_date_on,app.sanction_date_on as sanction_date_on,app.first_disb_date_on as first_disb_date_on,app_s.app_status as app_status, app_user.user_name as app_user_name from tbl_mint_app as app JOIN tbl_mint_case as c ON app.case_id = c.case_id JOIN tbl_mint_customer_info as cust ON c.cust_id = cust.id left join tbl_mlc_partner as partner on app.partner_on = partner.partner_id left join lms_loan_type as loan on c.loan_type = loan.loan_type_id left join tbl_user_assign as user on c.user_id = user.user_id left join tbl_bank as bank on app.app_bank_on = bank.bank_id left join tbl_app_status as app_s on app.app_status_on = app_s.app_status_id left join tbl_app_pre_login as pre_l on app.pre_login_status = pre_l.pre_status_id left join lms_city as city on cust.city_id = city.city_id left join tbl_application_feed_back as fb on app.app_id = fb.app_id LEFT JOIN tbl_user_assign AS app_user ON app_user.user_id = app.app_created_by ";
...
if($app_statussearch == ""){
    $qry_ex .= " and app.app_status_on != 8";
}
if($source_compign != "") {
...
<th width="10%">Post Login Status</th>
...
<td><?php echo "<span>".$name_app_statuson."</span>"; if($auto_case_create == 1){ echo "<br><span class='fs-12'> (Auto)</span>" ; } ?><br><?php echo $app_st_date;?></td>
