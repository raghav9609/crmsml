...
$select_name    = "";
$option_name    = "";
$on_change      = 'onchange="new_fcns_change(this, 2);"';
if($sub_level == 1) {
    $select_name    = "sub_status";
    $option_name    = "Select Sub Status";
} else if($sub_level == 2) {
    $select_name    = "sub_sub_status";
    $option_name    = "Select Sub Sub Status";
    $on_change      = "";
}

$get_status_query   = "SELECT status_id, description FROM status_master WHERE level_id = $level_id AND parent_id = $parent_id AND is_active_for_filter = 1 AND is_fos = 1 ";
