...
?>
   <?php $return_html = '<table width="100%" class="gridtable"><tr class="font-weight-bold"><th>Sr. No.</th><th>Application Id / Case Id </th><th>Action</th></tr>'; ?>
    ...
    ?>
    <?php 
    $return_html .= '<tr class="center-align"><td>'.$sr_no.'</td><td><a href="../app/edit_applicaton.php?case_id='.urlencode(base64_encode($case_id)).'&cust_id='.urlencode(base64_encode($cust_id)).$loan_type.'">'.$app_id.'</a><br><span class="fs-12">(<a href="../cases/edit.php?case_id='.urlencode(base64_encode($enc_cs)).'">'.$case_id.'</a>)</span></td><td style="width: 7%">'.$exe_app_bank['bank_name'].'<br><span class="fs-12">'.$partner_name.'</span></td></tr>'; 
    ?>
    <?php } ?>
