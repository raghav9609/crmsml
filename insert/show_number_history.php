...
$return_html = "";
    ...
    $number_history_qry = "select query_id, tbl_user_assign.name as user_name, datetime, phone_number_type from crm_show_number_history 
    ...
    $sr_no = 0;
    if($number_result > 0) {
        $return_html .= "<table  class='gridtable' width='100%'><tr><th>Sr. No.</th><th>Query Id</th><th>Phone Type</th><th>User</th><th>Date-Time</th></tr>";
        while($num_his_results = mysqli_fetch_array($number_history_res)) {
            ...
            $return_html .= "<tr class='center-align'><td>".$sr_no."</td><td>".$query_id."</td><td>".$phone_type."</td><td>".$user_name."</td><td>".$datetime."</td></tr>";
        }
        $return_html .= "</table>";
    }
